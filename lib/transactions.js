...
exports.createGeneration = function(rpcData, blockReward, feeReward, recipients, poolAddress){
    ...
    let bitIntFeePercent = BigInt(Math.floor(blockReward * (1 - (feePercent / 100))));
    tx.addOutput(
    ...
    for (var i = 0; i < recipients.length; i++) {
       let bitIntPercent = BigInt(Math.round((blockReward) * (recipients[i].percent / 100)));
       tx.addOutput(
    ...
    if (rpcData.default_witness_commitment !== undefined) {
        tx.addOutput(new Buffer(rpcData.default_witness_commitment, 'hex'), 0n);
    }
